...
class CountDownActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.currentTimeString.observe(this) { tvCountDown.text = it }
        viewModel.eventCountDownFinish.observe(this) {
            updateButtonState(!startCountDown && it)
            if (it && startCountDown) {
                ...
                workManager.getWorkInfoByIdLiveData(oneTimeWorkRequest.id).observe(this) { workInfo ->
                    if (workInfo.state == WorkInfo.State.ENQUEUED) {
                        Log.d(TAG, "Notification has been enqueued. Status: ${workInfo.state.name}")
