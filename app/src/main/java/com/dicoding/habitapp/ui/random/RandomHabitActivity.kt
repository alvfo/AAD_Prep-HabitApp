...
class RandomHabitActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.priorityLevelHigh.observe(this) { highPriorityHabit ->
            highPriorityHabit?.let {
                adapter.submitData(RandomHabitAdapter.PageType.HIGH, it)
            }
        }
        viewModel.priorityLevelMedium.observe(this) { mediumPriorityHabit ->
            mediumPriorityHabit?.let {
                adapter.submitData(RandomHabitAdapter.PageType.MEDIUM, it)
            }
        }
        viewModel.priorityLevelLow.observe(this) {lowPriorityHabit ->
            lowPriorityHabit?.let {
